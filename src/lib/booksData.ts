...
// Các hàm tiện ích tự động lấy dữ liệu
export function getBookBySlug(slug: string): Book | undefined {
  return BOOKS_DATA.find((b) => b.slug === slug);
}

export function getRelatedBooks(currentSlug: string, limit = 4): Book[] {
  return BOOKS_DATA.filter((b) => b.slug !== currentSlug).slice(0, limit);
}
