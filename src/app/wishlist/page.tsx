...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    setWishlist(JSON.parse(localStorage.getItem("bsWishlist") || "[]"));
  }, [user, router]);

  if (!user) return null;

  const removeWishlist = (idx: number) => {
    const updated = wishlist.filter((_, i) => i !== idx);
    setWishlist(updated);
    localStorage.setItem("bsWishlist", JSON.stringify(updated));
  };

  const handleAddToCart = (book: any) => {
    addItem({ id: book.slug || book.id || String(Math.random()), title: book.title, author: book.author, price: book.price, image: book.image, qty: 1 });
    setToast(true);
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                {wishlist.map((book, idx) => (
                  <div key={idx} className="border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    <div className="h-36 bg-linear-to-br from-lightBg to-accent flex items-center justify-center">
                      {book.image ? (
                        <img src={book.image} alt={book.title} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-16 h-24 bg-primary rounded shadow flex items-center justify-center">
                          <i className="fa-solid fa-book text-accent text-xl" />
                        </div>
                      )}
                    </div>
                    <div className="p-4">
                      <h3 className="font-bold text-primary text-sm leading-tight mb-1">{book.title}</h3>
                      <p className="text-secondary text-xs mb-2">{book.author || ""}</p>
                      <p className="text-danger font-bold">{book.price ? formatPrice(book.price) : ""}</p>
                      <div className="flex gap-2 mt-3">
                        <Link
                          href={`/products/${book.slug || ""}`}
                          className="flex-1 bg-primary text-white py-1.5 rounded-lg text-xs font-semibold hover:bg-secondary transition-colors text-center"
                        >
                          Xem
                        </Link>
                        <button
                          onClick={() => removeWishlist(idx)}
                          className="w-8 h-8 border border-danger text-danger rounded-lg flex items-center justify-center hover:bg-red-50 transition-colors"
                        >
