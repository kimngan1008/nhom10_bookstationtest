...
export default function BookDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  function handleAddToCart() {
    addItem({ id: book!.slug, title: book!.title, author: book!.author, price: book!.price, image: book!.image, qty });
    setToast(true);
      ...
      {relatedBooks.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {relatedBooks.map((b) => (
              <Link key={b.slug} href={`/products/${b.slug}`} className="bg-gray-50 rounded-xl p-3 border border-gray-100 hover:shadow-md transition-all cursor-pointer group flex flex-col">
                <div className="aspect-3/4 rounded-lg mb-2 flex items-center justify-center overflow-hidden bg-gray-100">
                  {b.image ? (
                ...
                <p className="font-semibold text-primary text-xs line-clamp-2 group-hover:text-accent transition-colors">{b.title}</p>
                <p className="text-[10px] text-secondary mt-1">{b.author}</p>
                <p className="text-yellow-400 text-[10px] mt-1">★★★★★ <span className="text-gray-400">({b.reviews})</span></p>
                <p className="text-danger font-bold text-xs mt-1">{formatPrice(b.price)}</p>
              </Link>
            ))}
          </div>
        </div>
