...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedCategories.length > 0)
      result = result.filter(b => selectedCategories.includes((b as any).category ?? ""));
    if (selectedLanguages.length > 0)
      result = result.filter(b => selectedLanguages.includes((b as any).language ?? "Tiếng Việt"));
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 md:gap-6">
              {paged.map(book => (
                <Link key={book.slug} href={`/products/${book.slug}`}
                  className="bg-white rounded-xl p-3 md:p-4 shadow-sm hover:shadow-md transition-all group flex flex-col border border-gray-50">
                  <div className="aspect-3/4 overflow-hidden rounded-lg mb-3 bg-gray-50 relative shrink-0">
                    ...
                    {book.discount && (
                      <span className="absolute top-2 left-2 bg-danger text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">
                        -{book.discount}%
                      </span>
                    )}
                  ...
                  <div className="flex flex-col flex-1">
                    <h3 className="font-heading text-base md:text-lg font-bold text-primary line-clamp-2 mb-1 group-hover:text-accent transition-colors leading-snug">
                      {book.title}
                    </h3>
                    <p className="text-[10px] md:text-xs text-gray-500 mb-2">{book.author}</p>
                    <div className="flex items-center gap-0.5 mb-2 mt-auto">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <i key={i} className={`fa-star text-[10px] ${i < book.rating ? "fa-solid text-yellow-400" : "fa-regular text-gray-300"}`} />
                      ))}
                      <span className="text-[10px] text-gray-400 ml-1">({book.reviews})</span>
                    </div>
                    <div className="flex items-baseline gap-2">
                      <p className="font-extrabold text-danger text-sm md:text-base">{formatPrice(book.price)}</p>
                      {book.originalPrice && (
                        <p className="text-[10px] text-gray-400 line-through">{formatPrice(book.originalPrice)}</p>
                      )}
                    </div>
                  </div>
                </Link>
