        ...
        <div className="max-w-6xl mx-auto bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-5 md:grid-cols-3 max-md:grid-cols-2 gap-6 md:gap-4">
            {flashSaleBooks.map((book) => (
              <Link key={book.slug} href={`/products/${book.slug}`} className="bg-white rounded-xl p-3 border border-gray-50 shadow-sm hover:shadow-xl transition-all group relative cursor-pointer flex flex-col">
                <span className="absolute top-3 left-3 bg-danger text-white text-[10px] font-bold px-2.5 py-1 rounded-full z-10">-{book.discount}%</span>
                <div className="overflow-hidden rounded-lg mb-4 bg-gray-50 aspect-3/4 shrink-0">
                ...
                <h4 className="font-heading font-bold text-primary text-[17px] line-clamp-1 max-md:text-sm">{book.title}</h4>
                <p className="text-xs text-secondary mb-2 mt-1 line-clamp-1">{book.author}</p>
                <div className="rating-box mb-3 max-md:text-base mt-auto">★★★★★<span className="rating-count">({book.reviews})</span></div>
                <div className="flex items-baseline gap-2">
                  <p className="text-danger font-extrabold text-xl tracking-tight max-md:text-lg">{formatPrice(book.price)}</p>
                  {book.originalPrice && <p className="text-[11px] text-gray-400 line-through max-md:hidden">{formatPrice(book.originalPrice)}</p>}
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="max-w-6xl mx-auto mt-12 mb-20 px-4">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-6 lg:grid-cols-6 md:grid-cols-4 max-md:grid-cols-2 gap-6">
            {newBooks.map((book) => (
              <Link key={book.slug} href={`/products/${book.slug}`} className="bg-white rounded-xl p-3 border border-gray-50 shadow-sm hover:shadow-xl transition-all group relative cursor-pointer flex flex-col">
                <span className="absolute top-3 left-3 bg-danger text-white text-[10px] font-bold px-2.5 py-1 rounded z-10 uppercase">Mới</span>
                <div className="overflow-hidden rounded-lg mb-4 bg-gray-50 aspect-3/4 shrink-0">
                  <img src={book.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={book.title} />
                </div>
                <h4 className="font-heading font-bold text-primary text-[17px] line-clamp-1 max-md:text-sm">{book.title}</h4>
