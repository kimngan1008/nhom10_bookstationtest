...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = () => {
    ...
    const existingOrders = JSON.parse(localStorage.getItem('bsOrders') || '[]');
    localStorage.setItem('bsOrders', JSON.stringify([newOrder, ...existingOrders]));
    
    if (paymentMethod === "momo") router.push("/checkout/momo");
    else if (paymentMethod === "bank") router.push("/checkout/bank-transfer");
    else router.push(`/orders/success`);
  };
