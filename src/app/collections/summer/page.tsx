  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-10 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 md:gap-6">
        {sorted.map((book) => (
          <Link key={book.slug} href={`/products/${book.slug}`} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-xl transition-all group flex flex-col relative overflow-hidden">
            <div className="absolute top-6 left-6 z-10 flex flex-col gap-1.5">
              {book.discount && (
                <span className="bg-danger text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">-{book.discount}%</span>
              )}
              <span className="bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">Mùa Hè</span>
            </div>

            <div className="aspect-3/4 overflow-hidden rounded-xl mb-4 bg-gray-50 shrink-0">
              {book.image ? (
            ...
            <div className="flex flex-col flex-1">
              <h3 className="font-bold text-primary text-sm line-clamp-2 mb-1 group-hover:text-accent transition-colors leading-snug">{book.title}</h3>
              <p className="text-xs text-secondary mb-2">{book.author}</p>
              <div className="mt-auto pt-3 flex items-end justify-between">
                <div>
                  <p className="font-extrabold text-danger text-base">{formatPrice(book.price)}</p>
                  {book.originalPrice && (
                    <p className="text-[11px] text-gray-400 line-through mt-0.5">{formatPrice(book.originalPrice)}</p>
                  )}
                </div>
                <button
