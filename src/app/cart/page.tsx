...
const VOUCHERS: Record<string, number> = { "BOOK10": 10, "SUMMER20": 20, "NEWUSER": 15 };
...
export default function CartPage() {
  ...
  function applyVoucher() {
    const pct = VOUCHERS[voucher.toUpperCase()];
    if (pct) { setDiscount(pct); setVoucherMsg(`✓ Áp dụng thành công! Giảm ${pct}%`); }
    else { setDiscount(0); setVoucherMsg("✗ Mã không hợp lệ hoặc đã hết hạn."); }
  }
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        ...
        <div className="lg:w-72 flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            {voucherMsg && <p className={`text-xs mt-2 ${discount > 0 ? "text-green-600" : "text-danger"}`}>{voucherMsg}</p>}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <button onClick={() => selected.length > 0 && router.push("/checkout")}
